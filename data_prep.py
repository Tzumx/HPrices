...
def data_norm(sample):
    """
    Preparing the data for models (One-hot encoding, Normalization, Handle Missing Values)
    """
    train_sample = sample.copy(deep=True)
    target_sample = pd.DataFrame([])
    if 'SalePrice' in train_sample.columns:
        target_sample['SalePrice'] = train_sample.pop('SalePrice')
    ...
    for colum in train_sample.columns.values.tolist():
        ...
        if pd.api.types.is_numeric_dtype(train_sample.loc[:, colum]):
            if train_sample.loc[:, colum].count() < train_sample.count().max():
                train_sample.loc[:, colum].fillna(
                    train_sample.loc[:, colum].mean(), inplace=True)
            train_sample.loc[:, colum] = minmax_scale(
                train_sample.loc[:, colum], (0, 100))
        else:
            if train_sample.loc[:, colum].count() < train_sample.count().max():
                train_sample.loc[:, colum].fillna(
                    train_sample.loc[:, colum].mode(), inplace=True)
            train_sample = pd.get_dummies(
                train_sample, columns=[colum], prefix=colum)
...
def data_filter(X_sample, Y_sample, cor_target=0.01, chi_target=250):
    """
    Filter data (drop out using correlation, chi2)
    """
    ...
    train_sample_drop = pd.Series(fit.scores_, index=train_sample.columns).nsmallest(
        len(train_sample.columns)-chi_target).index.values
    train_sample.drop(train_sample_drop, axis=1, inplace=True)
